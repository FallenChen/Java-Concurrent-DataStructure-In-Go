	...
	for {
		...
		if length == 0 {
			...
			case st.waitForNextElementChan <- waitChan:
				...
				// 
				for {
					// re-checks every i milliseconds (top : 10 times)... the following verifies if an item was 
					// enqueued around the same time DequeueOrWaitForNextElementContext was invoked, meaning the waitChan
					// wasn't yet sent over st.waitForNextElementChan
